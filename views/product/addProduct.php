...
<head>
    ...
    <style>
        ...
        form {
            width: 85%;
            /* Chiều rộng của form bằng 85% chiều rộng của body */
            margin: 0 auto;
            /* Căn giữa form trong body */
        ...
        label {
            font-weight: 600;
            /* Đặt độ đậm cho label */
            font-size: 1.1rem;
            /* Tăng kích thước chữ lên 1.1rem */
        }
        ...
        .form-select {
            ...
            /* Bo góc */
            padding: 10px;
            /* Padding cho thoải mái */
        }

        .form-control:focus,
        .form-select:focus {
            border-color: #b0b3b8;
            /* Màu viền xám đậm hơn một chút khi focus */
            box-shadow: 0 0 8px rgba(176, 179, 184, 0.5);
        ...
        @media (max-width: 768px) {
            form {
                width: 95%;
                /* Form chiếm 95% trên màn hình nhỏ */
            }
        }
    </style>
</head>
    ...
    <form action="?act=submitAddProduct" method="post" enctype="multipart/form-data">
        ...
        <div class="mb-3 row">
            ...
            <div class="col-md-6">
                ...
                <input type="number" class="form-control rounded-3 shadow-sm" id="discount_price" name="discount_price" min="0" value="<?= isset($_POST['discount_price']) ? htmlspecialchars($_POST['discount_price']) : '' ?>" placeholder="Nhập giá Giảm" required>
                <?php if (!empty($thongBaoLoidiscount_price)): ?>
                    <div class="alert alert-danger mt-2"><?= htmlspecialchars($thongBaoLoidiscount_price) ?></div>
                <?php endif; ?>
            </div>
        </div>
