...
<h1>Thêm mới Biến thể của sản phẩm: <?=$product["product_name"]?> </h1>
<form action="?act=submitAddProductVariant&id=<?=$product["id"]?>" method="post" enctype="multipart/form-data">
  <div class="mb-3">
    <label for="product_name" class="form-label">Tên sản phẩm:</label>
    <input type="text" class="form-control" id="product_name" name="product_name" value="<?= $product['product_name']?>" disabled required>
  </div>
  <div class="mb-3">
  ...
  <div class="mb-3">
    ...
    <input type="number" class="form-control" name="stock_quantity" min=0 value="<?= isset($_POST['stock_quantity']) ? htmlspecialchars($_POST['stock_quantity']) : '' ?>" placeholder="Mời nhập số lượng">
  ...
  <div class="mb-3">
    ...
    <input type="file" class="form-control" id="image_variant" name="image_variant" accept="image/*" value="<?= isset($_POST['price']) ? htmlspecialchars($_POST['image_variant']) : '' ?>" required>
